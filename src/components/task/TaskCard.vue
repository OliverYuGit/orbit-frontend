<template>
  ...
  >
    ...
    <div style="margin-top: 8px; display: flex; justify-content: space-between; align-items: center;">
      <span style="font-size: 12px; color: #646A73;">
        {{ task.assignee?.displayName ?? task.assignee?.username ?? 'Unassigned' }}
      </span>
      <n-dropdown :options="moveOptions" @select="(s) => $emit('move', s as TaskStatus)" trigger="click">
...
const moveOptions = STATUSES
  ...
  .map(s => ({ label: STATUS_LABELS[s], key: s }))
